...
typedef struct lua_checkstack_context_t
{
    int size;
} lua_checkstack_context_t;

static void __lua_checkstack_grow(lua_State* L, void* userdata)
{
    lua_checkstack_context_t* context = (lua_checkstack_context_t*)userdata;
    luaD_growstack(L, context->size);
}
...
#define ensure_stack_impl(L, errorL, size) \
    { \
        if (FFlag_LuauAutoStack && L->top + (size) > L->ci->top && !lua_checkstack(L, (size))) \
        { \
...
static lua_table_t* __getcurrenv(lua_State* L)
{
...
static LUAU_NOINLINE tvalue_t* __pseudo2addr(lua_State* L, int idx)
{
    ...
    {
        sethvalue(L, &L->global->pseudotemp, __getcurrenv(L));
    ...
    default:
    {
        closure_t* func = curr_func(L);
        idx = LUA_GLOBALSINDEX - idx;
        return (idx <= func->nupvalues) ? &func->c.upvals[idx - 1] : cast_to(tvalue_t*, luaO_nilobject);
    }
    }
}

static LUAU_FORCEINLINE tvalue_t* __index2addr(lua_State* L, int idx)
{
    if (idx > 0)
    {
        tvalue_t* o = L->base + (idx - 1);
        api_check(L, idx <= L->ci->top - L->base);
        if (o >= L->top)
            return cast_to(tvalue_t*, luaO_nilobject);
        else
            return o;
    }
    ...
    {
        return __pseudo2addr(L, idx);
    }
}

const tvalue_t* luaA_toobject(lua_State* L, int idx)
{
    StkId p = __index2addr(L, idx);
    return (p == luaO_nilobject) ? NULL : p;
}

void luaA_pushvalue(lua_State* L, const tvalue_t* o)
{
    ensure_stack(L, 1);
    setobj2s(L, L->top, o);
    api_incr_top(L);
...
void luaA_pushclass(lua_State* L, luauc_class_t* lco)
{
    ensure_stack(L, 1);
    api_check(L, lco != NULL);
...
int lua_checkstack(lua_State* L, int size)
{
    ...
    else if (size > 0)
    {
        ...
        {
            lua_checkstack_context_t context = {size};

            // there could be no memory to extend the stack
            if (luaD_rawrunprotected(L, __lua_checkstack_grow, &context) != LUA_OK)
                return 0;
        }
        else
        {
...
void lua_xpush(lua_State* from, lua_State* to, int idx)
{
    ...
    setobj2s(to, to->top, __index2addr(from, idx));
    api_incr_top(to);
}
...
{
    tvalue_t n;
    const tvalue_t* o = __index2addr(L, idx);
    return tonumber(o, &n);
}
...
{
    StkId o1 = __index2addr(L, index1);
    StkId o2 = __index2addr(L, index2);
...
int lua_equal(lua_State* L, int index1, int index2)
{
    StkId o1, o2;
    int i;
    o1 = __index2addr(L, index1);
    o2 = __index2addr(L, index2);
...
const char* lua_tolstring(lua_State* L, int idx, size_t* len)
{
    ...
    if (!ttisstring(o))
    {
        ...
        luaC_checkGC(L);
        o = __index2addr(L, idx); // previous call may reallocate the stack
    }
    if (len != NULL)
        *len = tsvalue(o)->len;
    return svalue(o);
...
void* lua_tobuffer(lua_State* L, int idx, size_t* len)
{
    ...
    luauc_vm_buffer_t* b = bufvalue(o);
...
void lua_pushcclosurek(lua_State* L, lua_CFunction fn, const char* debugname, int nup, lua_Continuation cont)
{
    ...
    closure_t* cl = luaF_newCclosure(L, nup, __getcurrenv(L));
    cl->c.f = fn;
    cl->c.cont = cont;
...
{
    const tvalue_t* o = __index2addr(L, objindex);
    api_check(L, ttistable(o));
    lua_table_t* t = hvalue(o);
...
int lua_getmetatable(lua_State* L, int objindex)
{
    ...
    ensure_stack(L, 1);
    lua_table_t* mt = NULL;
    const tvalue_t* obj = __index2addr(L, objindex);
    switch (ttype(obj))
    {
    case LUA_TTABLE:
...
{
    api_checknelems(L, 1);
    tvalue_t* obj = __index2addr(L, objindex);
...
*/
// data to `f_call'
struct lua_call_state_t
{
    StkId func;
    int nresults;
};

static void __f_call(lua_State* L, void* ud)
{
    struct lua_call_state_t* c = cast_to(struct lua_call_state_t*, ud);
...
int lua_pcall(lua_State* L, int nargs, int nresults, int errfunc)
{
    ...
    if (errfunc != 0)
    {
        StkId o = __index2addr(L, errfunc);
        api_checkvalidindex(L, o);
        func = savestack(L, o);
    }
    struct lua_call_state_t c;
    c.func = L->top - (nargs + 1); // function to be called
    c.nresults = nresults;

    int status = luaD_pcall(L, __f_call, &c, savestack(L, c.func), func);
...
*/
// data to `f_Ccall'
struct c_call_state_t
{
    lua_CFunction func;
    void* ud;
};

static void __f_Ccall(lua_State* L, void* ud)
{
    struct c_call_state_t* c = cast_to(struct c_call_state_t*, ud);

    if (!lua_checkstack(L, 2))
        luaG_runerror(L, "stack limit");

    lua_pushcclosurek(L, c->func, NULL, 0, NULL);
...
{
    api_check(L, L->status == 0);
    api_check(L, func != NULL);

    struct c_call_state_t c;
    c.func = func;
    c.ud = ud;

    return luaD_pcall(L, __f_Ccall, &c, savestack(L, L->top), 0);
}
...
int lua_gc(lua_State* L, int what, int data)
{
    ...
    case LUA_GCSTEP:
    {
        ...
        gc_cycle_metrics_t* cyclemetrics = g->gcstate == GCSpause ? &g->gcmetrics.lastcycle : &g->gcmetrics.currcycle;
...
int lua_rawiter(lua_State* L, int idx, int iter)
{
    ...
    {
        lua_node_t* n = &h->node[iter - sizearray];
...
void* lua_newuserdatatagged(lua_State* L, size_t sz, int tag)
{
    ...
    luaC_threadbarrier(L);
    ensure_stack(L, 1);
    udata_t* u = luaU_newudata(L, sz, tag);
    setuvalue(L, L->top, u);
    api_incr_top(L);
    return u->data;
}
...
void* lua_newuserdatataggedwithmetatable(lua_State* L, size_t sz, int tag)
{
    ...
    lua_table_t* h = L->global->udatamt[tag];
    api_check(L, h != NULL);
...
void* lua_newuserdatadtor(lua_State* L, size_t sz, void (*dtor)(void*))
{
    ...
    udata_t* u = luaU_newudata(L, as, UTAG_IDTOR);
...
void* lua_newbuffer(lua_State* L, size_t sz)
{
    ...
    luaC_threadbarrier(L);
    ensure_stack(L, 1);
    luauc_vm_buffer_t* b = luaB_newbuffer(L, sz);
    setbufvalue(L, L->top, b);
    api_incr_top(L);
    return b->data;
}

static const char* __aux_upvalue(StkId fi, int n, tvalue_t** val)
{
    closure_t* f;
    if (!ttisfunction(fi))
        return NULL;
...
const char* lua_getupvalue(lua_State* L, int funcindex, int n)
{
    ...
    ensure_stack(L, 1);
    tvalue_t* val;
    const char* name = __aux_upvalue(__index2addr(L, funcindex), n, &val);
    if (name)
    {
...
{
    api_checknelems(L, 1);
    StkId fi = __index2addr(L, funcindex);
    tvalue_t* val;
    const char* name = __aux_upvalue(fi, n, &val);
    if (name)
    {
        L->top--;
        setobj(L, val, L->top);
...
int lua_ref(lua_State* L, int idx)
{
    ...
    if (!ttisnil(p))
    {
        lua_table_t* reg = hvalue(registry(L));
        ...
        tvalue_t* slot = luaH_setnum(L, reg, ref);
        if (g->registryfree != 0)
...
void lua_getuserdatametatable(lua_State* L, int tag)
{
    ...
    lua_table_t* table = L->global->udatamt[tag];
    if (table != NULL)
    {
...
const char* lua_getuserdataname(lua_State* L, int tag)
{
    ...
    lua_table_t* metatable = L->global->udatamt[tag];
    if (metatable != NULL)
    {
        const tvalue_t* type = luaH_getstr(metatable, L->global->tmname[TM_TYPE]);
        if (ttisstring(type))
...
)
{
    ...
    if (table != NULL)
    {
        ...
        lua_udata_direct_access_data_t* udatadirect = &L->global->udatadirect[tag];

        const tvalue_t* indextm = fasttm(L, table, TM_INDEX);
        if (indextm != NULL)
        {
        ...
        const tvalue_t* newindextm = fasttm(L, table, TM_NEWINDEX);
        if (newindextm != NULL)
        {
        ...
        const tvalue_t* namecalltm = fasttm(L, table, TM_NAMECALL);
        if (namecalltm != NULL)
        {
...
void lua_clonefunction(lua_State* L, int idx)
{
    ...
    closure_t* cl = clvalue(p);
    closure_t* newcl = luaF_newLclosure(L, cl->nupvalues, L->gt, cl->l.p);
...
void lua_clonetable(lua_State* L, int idx)
{
    ...
    lua_table_t* tt = luaH_clone(L, hvalue(t));
