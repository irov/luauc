...
#define MAX_STACK_SIZE ((int)(1024 / sizeof(tvalue_t)) * 1024 * 1024)
...
static void __correctstack(lua_State* L, tvalue_t* oldstack)
{
    L->top = (L->top - oldstack) + L->stack;
    for (upvalue_t* up = L->openupval; up != NULL; up = up->u.open.threadnext)
...
void luaD_reallocstack(lua_State* L, int newsize, int fornewci)
{
    ...
    {
        // reallocation was performed to setup a new call_info_t frame, which we have to remove
        if (fornewci)
        {
    ...
    tvalue_t* oldstack = L->stack;
    int realsize = newsize + EXTRA_STACK;
    LUAU_ASSERT(L->stack_last - L->stack == L->stacksize - EXTRA_STACK);
    luaM_reallocarray(L, L->stack, L->stacksize, realsize, tvalue_t, L->memcat);
    tvalue_t* newstack = L->stack;
    for (int i = L->stacksize; i < realsize; i++)
        setnilvalue(newstack + i); // erase new segment
    L->stacksize = realsize;
    L->stack_last = newstack + newsize;
    __correctstack(L, oldstack);
}

void luaD_reallocCI(lua_State* L, int newsize)
{
    call_info_t* oldci = L->base_ci;
    luaM_reallocarray(L, L->base_ci, L->size_ci, newsize, call_info_t, L->memcat);
    L->size_ci = newsize;
...
call_info_t* luaD_growCI(lua_State* L)
{
...
static void __performcall(lua_State* L, StkId func, int nresults, bool preparereentry)
{
...
bool luaD_performcally(lua_State* L, StkId func, int nresults)
{
    ...
    __performcall(L, func, nresults, /* preparereentry */ false);

    if (L->status != LUA_OK)
    {
        call_info_t* caller = restoreci(L, cioffset);
...
void luaD_callint(lua_State* L, StkId func, int nresults, bool preparereentry)
{
    ...
    if (L->ci != L->base_ci)
    {
        closure_t* ccl = clvalue(L->ci->func);
    ...
    __performcall(L, func, nresults, preparereentry);
    ...
    if (fromyieldableccall)
    {
        ...
        // on yield, we have to set the call_info_t top of the C function including slots for expected results, to restore later
        if (yielded)
        {
            call_info_t* callerci = restoreci(L, cioffset);
...
static void __resume_continue(lua_State* L)
{
    ...
    while ((L->status == LUA_OK || L->status == SCHEDULED_REENTRY) && L->ci > L->base_ci)
    {
        ...
        if (cl->isC)
        {
            ...
            if (FFlag_LuauCustomYieldablePcalls)
            ...
            if (FFlag_LuauCustomYieldablePcalls && L->status == SCHEDULED_REENTRY)
                continue;
        ...
        {
            if (FFlag_LuauYieldIter2 && L->ci->flags & LUA_CALLINFO_OPYIELD)
                luau_finishop(L);
...
static void __resume(lua_State* L, void* ud)
{
...
static call_info_t* __resume_findhandler(lua_State* L)
{
    call_info_t* ci = L->ci;
...
static void __restore_stack_limit(lua_State* L)
{
    LUAU_ASSERT(L->stack_last - L->stack == L->stacksize - EXTRA_STACK);
    if (L->size_ci > LUAI_MAXCALLS)
...
static void __callerrfunc(lua_State* L, void* ud)
{
...
static void __resume_handle(lua_State* L, void* ud)
{
    call_info_t* ci = (call_info_t*)ud;
    ...
    if (FFlag_LuauCustomYieldablePcalls && ci->errfunc != 0)
    {
        ...
        int err = luaD_rawrunprotected(L, __callerrfunc, ci->base + (ci->errfunc - 1));
...
static int __resume_start(lua_State* L, lua_State* from, int nargs)
{
    api_check(L, nargs >= 0);
    api_check(L, L->top - L->base >= nargs);

    if (L->status != LUA_YIELD && L->status != LUA_BREAK && (L->status != 0 || L->ci != L->base_ci))
        return __resume_error(L, "cannot resume non-suspended coroutine", nargs);

    L->nCcalls = from ? from->nCcalls : 0;
    if (L->nCcalls >= LUAI_MAXCCALLS)
        return __resume_error(L, "C stack overflow", nargs);
...
static int __resume_finish(lua_State* L, int status, int oldnCcalls)
{
    call_info_t* ch = NULL;
    while (status != LUA_OK && (ch = __resume_findhandler(L)) != NULL)
    {
        ...
        status = luaD_rawrunprotected(L, __resume_handle, ch);
    }
...
{
    int starterror = __resume_start(L, from, nargs);
    if (starterror)
        return starterror;

    int oldnCcalls = L->nCcalls;

    int status = luaD_rawrunprotected(L, __resume, L->top - nargs);

    return __resume_finish(L, status, oldnCcalls);
}

int lua_resumeerror(lua_State* L, lua_State* from)
{
    int starterror = __resume_start(L, from, 1);
    if (starterror)
        return starterror;
    ...
    call_info_t* ci = __resume_findhandler(L);
    if (ci)
    {
        L->status = cast_byte(status);
        status = luaD_rawrunprotected(L, __resume_handle, ci);
    }
...
int luaD_pcall(lua_State* L, Pfunc func, void* u, ptrdiff_t old_top, ptrdiff_t ef)
{
    ...
    if (status != 0)
    {
        ...
        if (ef)
        {
            ...
            // if errfunc fails, we fail with "error in error handling" or "not enough memory"
            int err = luaD_rawrunprotected(L, __callerrfunc, restorestack(L, ef));
