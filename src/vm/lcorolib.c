...
static const char* const __statnames[] = {"running", "suspended", "normal", "dead", "dead"}; // dead appears twice for LUA_COERR and LUA_COFIN

static int __costatus(lua_State* L)
{
    lua_State* co = lua_tothread(L, 1);
    luaL_argexpected(L, co, 1, "thread");
    lua_pushstring(L, __statnames[lua_costatus(L, co)]);
    return 1;
}
...
static int __auxresume(lua_State* L, lua_State* co, int narg)
{
    ...
    if (co->status != LUA_YIELD)
    {
        ...
        if (status != LUA_COSUS)
        {
            lua_pushfstring(L, "cannot resume %s coroutine", __statnames[status]);
...
static int __interruptThread(lua_State* L, lua_State* co)
{
...
static int __auxresumecont(lua_State* L, lua_State* co)
{
...
static int __coresumefinish(lua_State* L, int r)
{
    if (r < 0)
    {
        lua_pushboolean(L, 0);
...
static int __coresumey(lua_State* L)
{
    lua_State* co = lua_tothread(L, 1);
    luaL_argexpected(L, co, 1, "thread");
    int narg = cast_int(L->top - L->base) - 1;
    int r = __auxresume(L, co, narg);

    if (r == CO_STATUS_BREAK)
        return __interruptThread(L, co);

    return __coresumefinish(L, r);
}
...
static int __coresumecont(lua_State* L, int status)
{
    ...
    int r = __auxresumecont(L, co);

    return __coresumefinish(L, r);
}

static int __auxwrapfinish(lua_State* L, int r)
{
    if (r < 0)
    {
        if (lua_isstring(L, -1))
...
static int __auxwrapy(lua_State* L)
{
...
static int __cocreate(lua_State* L)
{
...
static int __cowrap(lua_State* L)
{
    __cocreate(L);

    lua_pushcclosurek(L, __auxwrapy, NULL, 1, __auxwrapcont);
    return 1;
}

static int __coyield(lua_State* L)
{
    int nres = cast_int(L->top - L->base);
    return lua_yield(L, nres);
}

static int __corunning(lua_State* L)
{
    if (lua_pushthread(L))
...
static int __coyieldable(lua_State* L)
{
    lua_pushboolean(L, lua_isyieldable(L));
    return 1;
}

static int __coclose(lua_State* L)
{
    lua_State* co = lua_tothread(L, 1);
    luaL_argexpected(L, co, 1, "thread");
    ...
    if (status != LUA_COFIN && status != LUA_COERR && status != LUA_COSUS)
        luaL_error(L, "cannot close %s coroutine", __statnames[status]);
...
static const luaL_Reg __co_funcs[] = {
    {"create", __cocreate},
    {"running", __corunning},
    {"status", __costatus},
    {"wrap", __cowrap},
    {"yield", __coyield},
    {"isyieldable", __coyieldable},
    {"close", __coclose},
    {NULL, NULL},
};
...
{
    luaL_register(L, LUA_COLIBNAME, __co_funcs);

    lua_pushcclosurek(L, __coresumey, "resume", 0, __coresumecont);
