...
_Static_assert(offsetof(lua_node_t, val) == 0, "Unexpected Node memory layout, pointer cast in gval2slot is incorrect");

// table_key_t is bitpacked for memory efficiency so we need to validate bit counts for worst case
_Static_assert(LUA_TDEADKEY < (1 << 4), "not enough bits for tt");
_Static_assert(MAXBITS < 28, "not enough bits for next");

// empty hash data points to dummynode so that we can always dereference it
const lua_node_t luaH_dummynode = {
...
static lua_node_t* __hashpointer(const lua_table_t* t, const void* p)
{
...
static lua_node_t* __hashnum(const lua_table_t* t, double n)
{
...
static lua_node_t* __hashint(const lua_table_t* t, int64_t n)
{
...
static lua_node_t* __hashvec(const lua_table_t* t, const float* v)
{
...
** of its hash value)
*/
static lua_node_t* __mainposition(const lua_table_t* t, const tvalue_t* key)
{
    switch (ttype(key))
    {
    case LUA_TNUMBER:
        return __hashnum(t, nvalue(key));
    case LUA_TINTEGER:
        return __hashint(t, lvalue(key));
    case LUA_TVECTOR:
        return __hashvec(t, vvalue(key));
    case LUA_TSTRING:
        return hashstr(t, tsvalue(key));
    case LUA_TBOOLEAN:
        return hashboolean(t, bvalue(key));
    case LUA_TLIGHTUSERDATA:
        return __hashpointer(t, pvalue(key));
    default:
        return __hashpointer(t, gcvalue(key));
    }
}
...
*/
static int __arrayindex(double key)
{
    int i;
    luai_num2int(i, key);
...
*/
static int __findindex(lua_State* L, lua_table_t* t, StkId key)
{
    int i;
    if (ttisnil(key))
        return -1; // first iteration
    i = ttisnumber(key) ? __arrayindex(nvalue(key)) : -1;
...
int luaH_next(lua_State* L, lua_table_t* t, StkId key)
{
    int i = __findindex(L, t, key); // find original element
...
static int __computesizes(int nums[], int* narray)
{
    int i;
    int twotoi; // 2^i
...
static int __countint(double key, int* nums)
{
    int k = __arrayindex(key);
    if (0 < k && k <= MAXSIZE)
...
static int __numusearray(const lua_table_t* t, int* nums)
{
    int lg;
    int ttlg;     // 2^lg
...
static int __numusehash(const lua_table_t* t, int* nums, int* pnasize)
{
    ...
    while (i--)
    {
        lua_node_t* n = &t->node[i];
        if (!ttisnil(gval(n)))
        {
            if (ttisnumber(gkey(n)))
                ause += __countint(nvalue(gkey(n)), nums);
            totaluse++;
        }
    }
    *pnasize += ause;
...
static void __setarrayvector(lua_State* L, lua_table_t* t, int size)
{
    if (size > MAXSIZE)
        luaG_runerror(L, "table overflow");
    luaM_reallocarray(L, t->array, t->sizearray, size, tvalue_t, t->memcat);
...
static void __setnodevector(lua_State* L, lua_table_t* t, int size)
{
    int lsize;
    if (size == 0)
    {                                           // no elements to hash part?
        t->node = cast_to(lua_node_t*, dummynode); // use common `dummynode'
        lsize = 0;
    }
    else
    {
        ...
        if (lsize > MAXBITS)
            luaG_runerror(L, "table overflow");
        size = twoto(lsize);
        t->node = luaM_newarray(L, size, lua_node_t, t->memcat);
        for (i = 0; i < size; i++)
        {
            lua_node_t* n = gnode(t, i);
            gnext(n) = 0;
            setnilvalue(gkey(n));
            setnilvalue(gval(n));
        }
...
static tvalue_t* __newkey(lua_State* L, lua_table_t* t, const tvalue_t* key);

static tvalue_t* __arrayornewkey(lua_State* L, lua_table_t* t, const tvalue_t* key)
{
    if (ttisnumber(key))
    {
        int k;
    ...
    return __newkey(L, t, key);
}

static void __resize(lua_State* L, lua_table_t* t, int nasize, int nhsize)
{
    if (nasize > MAXSIZE || nhsize > MAXSIZE)
        luaG_runerror(L, "table overflow");
    int oldasize = t->sizearray;
    int oldhsize = t->lsizenode;
    lua_node_t* nold = t->node; // save old hash ...
    if (nasize > oldasize)   // array part must grow?
        __setarrayvector(L, t, nasize);
    // create new hash part with appropriate size
    __setnodevector(L, t, nhsize);
    // used for the migration check at the end
    lua_node_t* nnew = t->node;
    if (nasize < oldasize)
    { // array part must shrink?
        ...
        for (int i = nasize; i < oldasize; i++)
        {
            if (!ttisnil(&t->array[i]))
            {
                ...
                setobjt2t(L, __newkey(L, t, &ok), &t->array[i]);
            }
        }
        // shrink array
        luaM_reallocarray(L, t->array, oldasize, nasize, tvalue_t, t->memcat);
    }
    // used for the migration check at the end
    tvalue_t* anew = t->array;
    // re-insert elements from hash part
    for (int i = twoto(oldhsize) - 1; i >= 0; i--)
    {
        lua_node_t* old = nold + i;
        if (!ttisnil(gval(old)))
        {
            tvalue_t ok;
            getnodekey(L, &ok, old);
            setobjt2t(L, __arrayornewkey(L, t, &ok), gval(old));
        }
    }
    ...
    if (nold != dummynode)
        luaM_freearray(L, nold, twoto(oldhsize), lua_node_t, t->memcat); // free old array
}

static int __adjustasize(lua_table_t* t, int size, const tvalue_t* ek)
{
    bool tbound = t->node != dummynode || size < t->sizearray;
    int ekindex = ek && ttisnumber(ek) ? __arrayindex(nvalue(ek)) : -1;
...
void luaH_resizearray(lua_State* L, lua_table_t* t, int nasize)
{
    int nsize = (t->node == dummynode) ? 0 : sizenode(t);
    int asize = __adjustasize(t, nasize, NULL);
    __resize(L, t, asize, nsize);
}

void luaH_resizehash(lua_State* L, lua_table_t* t, int nhsize)
{
    __resize(L, t, t->sizearray, nhsize);
}

static void __rehash(lua_State* L, lua_table_t* t, const tvalue_t* ek)
{
    int nums[MAXBITS + 1]; // nums[i] = number of keys between 2^(i-1) and 2^i
    for (int i = 0; i <= MAXBITS; i++)
        nums[i] = 0;                          // reset counts
    int nasize = __numusearray(t, nums);        // count keys in array part
    int totaluse = nasize;                    // all those keys are integer keys
    totaluse += __numusehash(t, nums, &nasize); // count keys in hash part

    // count extra key
    if (ttisnumber(ek))
        nasize += __countint(nvalue(ek), nums);
    totaluse++;

    // compute new size for array part
    int na = __computesizes(nums, &nasize);
    int nh = totaluse - na;

    // enforce the boundary invariant; for performance, only do hash lookups if we must
    int nadjusted = __adjustasize(t, nasize, ek);
    ...
    if (aextra != 0)
    {
        ...
        nasize = __adjustasize(t, nasize, ek);
    }

    // resize the table to new computed sizes
    __resize(L, t, nasize, nh);
}
...
lua_table_t* luaH_new(lua_State* L, int narray, int nhash)
{
    lua_table_t* t = luaM_newgco(L, lua_table_t, sizeof(lua_table_t), L->activememcat);
    ...
    t->safeenv = 0;
    t->nodemask8 = 0;
    t->node = cast_to(lua_node_t*, dummynode);
    if (narray > 0)
        __setarrayvector(L, t, narray);
    if (nhash > 0)
        __setnodevector(L, t, nhash);
    return t;
}

void luaH_free(lua_State* L, lua_table_t* t, lua_page_t* page)
{
    if (t->node != dummynode)
        luaM_freearray(L, t->node, sizenode(t), lua_node_t, t->memcat);
    if (t->array)
        luaM_freearray(L, t->array, t->sizearray, tvalue_t, t->memcat);
    luaM_freegco(L, t, sizeof(lua_table_t), t->memcat, page);
}

static lua_node_t* __getfreepos(lua_table_t* t)
{
    while (t->lastfree > 0)
    {
        t->lastfree--;

        lua_node_t* n = gnode(t, t->lastfree);
        if (ttisnil(gkey(n)))
            return n;
    }
...
static tvalue_t* __newkey(lua_State* L, lua_table_t* t, const tvalue_t* key)
{
    ...
    {
        __rehash(L, t, key); // grow table

        // after rehash, numeric keys might be located in the new array part, but won't be found in the node part
        return __arrayornewkey(L, t, key);
    }

    lua_node_t* mp = __mainposition(t, key);
    if (!ttisnil(gval(mp)) || mp == dummynode)
    {
        lua_node_t* n = __getfreepos(t); // get a free place
        if (n == NULL)
        ...
        tvalue_t mk;
        getnodekey(L, &mk, mp);
        lua_node_t* othern = __mainposition(t, &mk);
        if (othern != mp)
...
*/
const tvalue_t* luaH_getnum(lua_table_t* t, int key)
{
...
*/
const tvalue_t* luaH_getstr(lua_table_t* t, tstring_t* key)
{
    lua_node_t* n = hashstr(t, key);
    for (;;)
    { // check whether `key' is somewhere in the chain
...
*/
const tvalue_t* luaH_getp(lua_table_t* t, void* key, int tag)
{
    lua_node_t* n = __hashpointer(t, key);
    for (;;)
    { // check whether `key' is somewhere in the chain
        const table_key_t* nk = gkey(n);
...
*/
const tvalue_t* luaH_get(lua_table_t* t, const tvalue_t* key)
{
    switch (ttype(key))
    {
    case LUA_TNIL:
...
{
    const tvalue_t* p = luaH_get(t, key);
    invalidateTMcache(t);
    if (p != luaO_nilobject)
        return cast_to(tvalue_t*, p);
    else
        return luaH_newkey(L, t, key);
}

tvalue_t* luaH_newkey(lua_State* L, lua_table_t* t, const tvalue_t* key)
{
    if (ttisnil(key))
...
tvalue_t* luaH_setnum(lua_State* L, lua_table_t* t, int key)
{
    // (1 <= key && key <= t->sizearray)
    if (((unsigned)(key)) - 1 < ((unsigned)(t->sizearray)))
        return &t->array[key - 1];
    // hash fallback
    const tvalue_t* p = luaH_getnum(t, key);
    if (p != luaO_nilobject)
        return cast_to(tvalue_t*, p);
    else
    {
        ...
        return __newkey(L, t, &k);
    }
}

tvalue_t* luaH_setstr(lua_State* L, lua_table_t* t, tstring_t* key)
{
    const tvalue_t* p = luaH_getstr(t, key);
    invalidateTMcache(t);
    if (p != luaO_nilobject)
        return cast_to(tvalue_t*, p);
    else
...
tvalue_t* luaH_setp(lua_State* L, lua_table_t* t, void* key, int tag)
{
    const tvalue_t* p = luaH_getp(t, key, tag);
    if (p != luaO_nilobject)
        return cast_to(tvalue_t*, p);
    else
    {
...
static int __updateaboundary(lua_table_t* t, int boundary)
{
...
int luaH_getn(lua_table_t* t)
{
    ...
    if (boundary > 0)
    {
        ...
        int foundboundary = __updateaboundary(t, boundary);
        if (foundboundary > 0)
...
lua_table_t* luaH_clone(lua_State* L, lua_table_t* tt)
{
    lua_table_t* t = luaM_newgco(L, lua_table_t, sizeof(lua_table_t), L->activememcat);
    luaC_init(L, t, LUA_TTABLE);
    ...
    if (tt->sizearray)
    {
        t->array = luaM_newarray(L, tt->sizearray, tvalue_t, t->memcat);
        maybesetaboundary(t, getaboundary(tt));
        t->sizearray = tt->sizearray;

        memcpy(t->array, tt->array, t->sizearray * sizeof(tvalue_t));
    }
    ...
    if (tt->node != dummynode)
    {
        ...
        memcpy(t->node, tt->node, size * sizeof(lua_node_t));
...
void luaH_clear(lua_table_t* tt)
{
    // clear array part
