...
static int __u_posrelat(int pos, size_t len)
{
    if (pos >= 0)
        return pos;
...
*/
static const char* __utf8_decode(const char* o, int* val)
{
    static const unsigned int __limits[] = {0xFF, 0x7F, 0x7FF, 0xFFFF};
    ...
    else
    {
        ...
        if (count > 3 || res > MAXUNICODE || res <= __limits[count])
            return NULL; // invalid byte sequence
...
** that interval
*/
static int __utflen(lua_State* L)
{
    int n = 0;
    size_t len;
    const char* s = luaL_checklstring(L, 1, &len);
    int posi = __u_posrelat(luaL_optinteger(L, 2, 1), len);
    int posj = __u_posrelat(luaL_optinteger(L, 3, -1), len);
    ...
    while (posi <= posj)
    {
        const char* s1 = __utf8_decode(s + posi, NULL);
        if (s1 == NULL)
        {                                 // conversion error?
...
*/
static int __codepoint(lua_State* L)
{
    size_t len;
    const char* s = luaL_checklstring(L, 1, &len);
    int posi = __u_posrelat(luaL_optinteger(L, 2, 1), len);
    int pose = __u_posrelat(luaL_optinteger(L, 3, posi), len);
    int n;
    const char* se;
...
static int __luaO_utf8esc(char* buff, unsigned long x)
{
...
static int __buffutfchar(lua_State* L, int arg, char* buff, const char** charstr)
{
    int code = luaL_checkinteger(L, arg);
    luaL_argcheck(L, 0 <= code && code <= MAXUNICODE, arg, "value out of range");
    int l = __luaO_utf8esc(buff, cast_to(long, code));
...
*/
static int __utfchar(lua_State* L)
{
    ...
    { // optimize common case of single char
        int l = __buffutfchar(L, 1, buff, &charstr);
    ...
    else
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            int l = __buffutfchar(L, i, buff, &charstr);
...
*/
static int __byteoffset(lua_State* L)
{
    size_t len;
    const char* s = luaL_checklstring(L, 1, &len);
    int n = luaL_checkinteger(L, 2);
    int posi = (n >= 0) ? 1 : (int)len + 1;
    posi = __u_posrelat(luaL_optinteger(L, 3, posi), len);
...
static int __iter_aux(lua_State* L)
{
    size_t len;
    const char* s = luaL_checklstring(L, 1, &len);
    ...
    {
        int code;
        const char* next = __utf8_decode(s + n, &code);
...
static int __iter_codes(lua_State* L)
{
    luaL_checkstring(L, 1);
    lua_pushcfunction(L, __iter_aux, NULL);
    lua_pushvalue(L, 1);
    lua_pushinteger(L, 0);
    return 3;
}
...
{
    luaL_register(L, LUA_UTF8LIBNAME, __funcs);
