...
#define LUAUC_POW10_TABLE_MIN (-292)
#define LUAUC_POW10_TABLE_MAX 324
#define LUAUC_POW10_TABLE_ROWS ((LUAUC_POW10_TABLE_MAX - LUAUC_POW10_TABLE_MIN + 1 + 15) / 16)

// clang-format off
static const uint64_t __kPow5Table[16] = {
...
static const uint64_t __kPow10Table[LUAUC_POW10_TABLE_ROWS][3] = {
...
static const char __kDigitTable[] = "0001020304050607080910111213141516171819202122232425262728293031323334353637383940414243444546474849"
                                  "5051525354555657585960616263646566676869707172737475767778798081828384858687888990919293949596979899";

// x*y => 128-bit product (lo+hi)
static inline uint64_t __mul128(uint64_t x, uint64_t y, uint64_t* hi)
{
...
static inline uint64_t __mul192hi(uint64_t xhi, uint64_t xlo, uint64_t y, uint64_t* hi)
{
    uint64_t z2;
    uint64_t z1 = __mul128(xhi, y, &z2);

    uint64_t z1c;
    uint64_t z0 = __mul128(xlo, y, &z1c);
    (void)z0;
...
static inline uint64_t __roundodd(uint64_t ghi, uint64_t glo, uint64_t cp)
{
    uint64_t xhi;
    uint64_t xlo = __mul128(glo, cp, &xhi);
    (void)xlo;

    uint64_t yhi;
    uint64_t ylo = __mul128(ghi, cp, &yhi);
...
static decimal_t __schubfach(int exponent, uint64_t fraction)
{
    ...
    LUAU_ASSERT(-k >= LUAUC_POW10_TABLE_MIN && -k <= LUAUC_POW10_TABLE_MAX);
    int gtoff = -k - LUAUC_POW10_TABLE_MIN;
    const uint64_t* gt = __kPow10Table[gtoff >> 4];

    uint64_t ghi;
    uint64_t glo = __mul192hi(gt[0], gt[1], __kPow5Table[gtoff & 15], &ghi);
    ...
    uint64_t vbl = __roundodd(ghi, glo, cbl << h);
    uint64_t vb = __roundodd(ghi, glo, cb << h);
    uint64_t vbr = __roundodd(ghi, glo, cbr << h);
    ...
    return (decimal_t){s + (uin != win ? win : rup), k};
}

static char* __printspecial(char* buf, int sign, uint64_t fraction)
{
    if (fraction == 0)
    {
...
static char* __printunsignedrev(char* end, uint64_t num)
{
    while (num >= 10000)
    {
        unsigned int tail = ((unsigned)(num % 10000));

        memcpy(end - 4, &__kDigitTable[((int)(tail / 100)) * 2], 2);
        memcpy(end - 2, &__kDigitTable[((int)(tail % 100)) * 2], 2);
        num /= 10000;
        end -= 4;
    }
    ...
    while (rest >= 10)
    {
        memcpy(end - 2, &__kDigitTable[((int)(rest % 100)) * 2], 2);
        rest /= 100;
        end -= 2;
    }
...
static char* __printexp(char* buf, int num)
{
    *buf++ = 'e';
    *buf++ = num < 0 ? '-' : '+';
    ...
    memcpy(buf, &__kDigitTable[v * 2], 2);
    return buf + 2;
}

static inline char* __trimzero(char* end)
{
    while (end[-1] == '0')
        end--;
...
char* luai_num2str(char* buf, double n)
{
    ...
    if (LUAU_UNLIKELY(exponent == 0x7ff))
        return __printspecial(buf, sign, fraction);
    ...
    *buf = '-';
    ...
    decimal_t d = __schubfach(exponent, fraction);
    ...
    char* dec = __printunsignedrev(decend, d.s);
    ...
    if (dot >= -5 && dot <= 21)
    {
        ...
        if (dot <= 0)
        {
            ...
            return __trimzero(buf + 2 + (-dot) + declen);
        }
        else if (dot == declen)
        {
            // no dot
        ...
        else if (dot < declen)
        {
            ...
            return __trimzero(buf + declen + 1);
        }
        else
        {
            // no dot, zero padding
    ...
    else
    {
        ...
        char* exp = __trimzero(buf + declen + 1);

        if (exp[-1] == '.')
            exp--;

        return __printexp(exp, dot - 1);
    }
}
