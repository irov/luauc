...
#ifndef LUAUC_BYTECODE_H
#define LUAUC_BYTECODE_H
...
// Version 2: Adds proto_t.linedefined. Supported until 0.544.
...
// Version 4: Adds proto_t.flags, typeinfo, and floor division opcodes IDIV/IDIVK. Currently supported.
...
typedef enum luauc_opcode_t
{
    ...
    // A: builtin function id (see luauc_builtin_function_t)
    ...
    // A: capture type, see luauc_capture_type_t
...
typedef enum luauc_bytecode_tag_t
{
    ...
    LBC_CONSTANT__COUNT
} luauc_bytecode_tag_t;

// Type table tags
typedef enum luauc_bytecode_type_t
{
    LBC_TYPE_NIL = 0,
    LBC_TYPE_BOOLEAN,
    LBC_TYPE_NUMBER,
...
} luauc_bytecode_type_t;

// Builtin function ids, used in LOP_FASTCALL
typedef enum luauc_builtin_function_t
{
    LBF_NONE = 0,
...
} luauc_builtin_function_t;

// Capture type, used in LOP_CAPTURE
typedef enum luauc_capture_type_t
{
    LCT_VAL = 0,
    LCT_REF,
    LCT_UPVAL,
} luauc_capture_type_t;

// proto_t flag bitmask, stored in proto_t.flags
typedef enum luauc_proto_flag_t
{
...
typedef enum luauc_feedback_type_t
{
    LFT_CALLTARGET = 0
} luauc_feedback_type_t;
