...
)
{
    ...
    luauc_class_t* classobject = luaM_newgco(L, luauc_class_t, sizeof(luauc_class_t), L->activememcat);
    luaC_init(L, classobject, LUA_TCLASS);
    classobject->name = name;

    classobject->staticmembers = luaM_newarray(L, numberofstaticmembers, tvalue_t, classobject->memcat);
    ...
    // environment.
    closure_t* constructor = luaF_newCclosure(L, 0, L->gt);
    constructor->c.f = luaR_createobject;
    constructor->c.debugname = "luaR_createobject";
    constructor->c.cont = NULL;
    tvalue_t* dest = luaH_setstr(L, classobject->metatable, L->global->tmname[TM_CALL]);
...
void luaR_addclassmember(lua_State* L, luauc_class_t* classobject, tstring_t* name, tvalue_t* value)
{
    LUAU_ASSERT(classobject->staticmembers != NULL);
    const tvalue_t* offset = luaH_getstr(classobject->memberstooffset, name);
    ...
    if (isMetamethod)
    {
        ...
        tvalue_t* dest = luaH_setstr(L, classobject->instancemetatable, name);
...
int luaR_createobject(lua_State* L)
{
    ...
    luauc_class_t* classobject = classvalue(L->base);
    luauc_object_t* classinst = luaM_newgco(L, luauc_object_t, sizeof(luauc_object_t), L->activememcat);
    ...
    classinst->members = luaM_newarray(L, classinst->numberofmembers, tvalue_t, L->activememcat);
...
void luaR_freeclass(lua_State* L, luauc_class_t* classobject, lua_page_t* page)
{
    luaM_freearray(
        L, classobject->staticmembers, classobject->numberofallmembers - classobject->numberofinstancemembers, tvalue_t, classobject->memcat
    );
    luaM_freearray(L, classobject->offsettomember, classobject->numberofallmembers, tstring_t*, classobject->memcat);
    luaM_freegco(L, classobject, sizeof(luauc_class_t), classobject->memcat, page);
}

void luaR_freeobject(lua_State* L, luauc_object_t* classinstance, lua_page_t* page)
{
    luaM_freearray(L, classinstance->members, classinstance->numberofmembers, tvalue_t, classinstance->memcat);
    luaM_freegco(L, classinstance, sizeof(luauc_object_t), classinstance->memcat, page);
}
