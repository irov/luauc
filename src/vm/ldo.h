...
#define stacklimitreached(L, n) ((char*)L->stack_last - (char*)L->top <= (n) * (int)sizeof(tvalue_t))
...
#define restorestack(L, n) check_exp((n) >= 0 && (size_t)(n) <= sizeof(tvalue_t) * L->stacksize, (tvalue_t*)((char*)L->stack + (n)))
...
#define restoreci(L, n) check_exp((n) >= 0 && (size_t)(n) <= sizeof(call_info_t) * L->size_ci, (call_info_t*)((char*)L->base_ci + (n)))
...
LUAI_FUNC call_info_t* luaD_growCI(lua_State* L);
