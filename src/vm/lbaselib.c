...
static void __writestring(const char* s, size_t l)
{
    fwrite(s, 1, l, stdout);
}

static int __luaB_print(lua_State* L)
{
...
static int __luaB_tonumber(lua_State* L)
{
...
static int __luaB_error(lua_State* L)
{
...
static int __luaB_getmetatable(lua_State* L)
{
    luaL_checkany(L, 1);
...
static int __luaB_setmetatable(lua_State* L)
{
    int t = lua_type(L, 2);
...
static void __getfunc(lua_State* L, int opt)
{
...
static int __luaB_getfenv(lua_State* L)
{
    __getfunc(L, 1);
...
static int __luaB_setfenv(lua_State* L)
{
...
static int __luaB_rawequal(lua_State* L)
{
    luaL_checkany(L, 1);
    luaL_checkany(L, 2);
...
static int __luaB_rawget(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checkany(L, 2);
    lua_settop(L, 2);
...
static int __luaB_rawset(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checkany(L, 2);
    luaL_checkany(L, 3);
...
static int __luaB_rawlen(lua_State* L)
{
    int tt = lua_type(L, 1);
...
static int __luaB_gcinfo(lua_State* L)
{
    lua_pushinteger(L, lua_gc(L, LUA_GCCOUNT, 0));
    return 1;
}

static int __luaB_type(lua_State* L)
{
    luaL_checkany(L, 1);
...
static int __luaB_typeof(lua_State* L)
{
    luaL_checkany(L, 1);
...
static int __luaB_pairs(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
...
static int __luaB_ipairs(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    lua_pushvalue(L, lua_upvalueindex(1)); // return generator,
    lua_pushvalue(L, 1);                   // state,
...
static int __luaB_assert(lua_State* L)
{
    luaL_checkany(L, 1);
...
static int __luaB_select(lua_State* L)
{
    int n = lua_gettop(L);
...
static void __luaB_pcallrun(lua_State* L, void* ud)
{
    LUAU_ASSERT(!FFlag_LuauCustomYieldablePcalls);
...
static int __luaB_pcally(lua_State* L)
{
    luaL_checkany(L, 1);
    ...
    else
    {
        ...
        int status = luaD_pcall(L, __luaB_pcallrun, func, savestack(L, func), 0);
...
static int __luaB_pcallcont(lua_State* L, int status)
{
    if (status == 0)
    {
        lua_rawcheckstack(L, 1);
...
static int __luaB_xpcally(lua_State* L)
{
    ...
    else
    {
        ...
        int status = luaD_pcall(L, __luaB_pcallrun, func, savestack(L, func), savestack(L, errf));
...
static void __luaB_xpcallerr(lua_State* L, void* ud)
{
    LUAU_ASSERT(!FFlag_LuauCustomYieldablePcalls);
...
static int __luaB_xpcallcont(lua_State* L, int status)
{
    if (status == 0)
    {
        lua_rawcheckstack(L, 1);
    ...
    else if (FFlag_LuauCustomYieldablePcalls)
    {
        lua_rawcheckstack(L, 1);
        lua_pushboolean(L, false);
    ...
    else
    {
        ...
        int err = luaD_pcall(L, __luaB_xpcallerr, errf, oldtopoffset, 0);
...
static int __luaB_tostring(lua_State* L)
{
    luaL_checkany(L, 1);
...
static int __luaB_newproxy(lua_State* L)
{
    int t = lua_type(L, 1);
...
static const luaL_Reg __base_funcs[] = {
    {"assert", __luaB_assert},
    {"error", __luaB_error},
    {"gcinfo", __luaB_gcinfo},
    {"getfenv", __luaB_getfenv},
    {"getmetatable", __luaB_getmetatable},
    {"next", luaB_next},
    {"newproxy", __luaB_newproxy},
    {"print", __luaB_print},
    {"rawequal", __luaB_rawequal},
    {"rawget", __luaB_rawget},
    {"rawset", __luaB_rawset},
    {"rawlen", __luaB_rawlen},
    {"select", __luaB_select},
    {"setfenv", __luaB_setfenv},
    {"setmetatable", __luaB_setmetatable},
    {"tonumber", __luaB_tonumber},
    {"tostring", __luaB_tostring},
    {"type", __luaB_type},
    {"typeof", __luaB_typeof},
    {NULL, NULL},
};

static void __auxopen(lua_State* L, const char* name, lua_CFunction f, lua_CFunction u)
{
...
int luaopen_base(lua_State* L)
{
    ...
    luaL_register(L, "_G", __base_funcs);
    lua_pushliteral(L, "Luau");
    lua_setglobal(L, "_VERSION"); // set global _VERSION

    // `ipairs' and `pairs' need auxiliary functions as upvalues
    __auxopen(L, "ipairs", __luaB_ipairs, luaB_inext);
    __auxopen(L, "pairs", __luaB_pairs, luaB_next);

    lua_pushcclosurek(L, __luaB_pcally, "pcall", 0, __luaB_pcallcont);
    lua_setfield(L, -2, "pcall");

    lua_pushcclosurek(L, __luaB_xpcally, "xpcall", 0, __luaB_xpcallcont);
