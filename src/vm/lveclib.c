...
static int __vector_create(lua_State* L)
{
...
static int __vector_magnitude(lua_State* L)
{
...
static int __vector_normalize(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_cross(lua_State* L)
{
...
static int __vector_dot(lua_State* L)
{
    const float* a = luaL_checkvector(L, 1);
    const float* b = luaL_checkvector(L, 2);
...
static int __vector_angle(lua_State* L)
{
    const float* a = luaL_checkvector(L, 1);
    const float* b = luaL_checkvector(L, 2);
...
static int __vector_floor(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_ceil(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_abs(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_sign(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
#else
    lua_pushvector(L, __luaui_signf(v[0]), __luaui_signf(v[1]), __luaui_signf(v[2]));
#endif

    return 1;
}

static int __vector_clamp(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
...
#else
    lua_pushvector(L, __luaui_clampf(v[0], min[0], max[0]), __luaui_clampf(v[1], min[1], max[1]), __luaui_clampf(v[2], min[2], max[2]));
#endif

    return 1;
}

static int __vector_min(lua_State* L)
{
    int n = lua_gettop(L);
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_max(lua_State* L)
{
    int n = lua_gettop(L);
    const float* v = luaL_checkvector(L, 1);
...
static int __vector_index(lua_State* L)
{
    const float* v = luaL_checkvector(L, 1);
    size_t namelen = 0;
...
static int __vector_lerp(lua_State* L)
{
    const float* a = luaL_checkvector(L, 1);
    const float* b = luaL_checkvector(L, 2);
...
#else
    lua_pushvector(L, __luai_lerpf(a[0], b[0], t), __luai_lerpf(a[1], b[1], t), __luai_lerpf(a[2], b[2], t));
#endif

    return 1;
}

static const luaL_Reg __vectorlib[] = {
    {"create", __vector_create},
    {"magnitude", __vector_magnitude},
    {"normalize", __vector_normalize},
    {"cross", __vector_cross},
    {"dot", __vector_dot},
    {"angle", __vector_angle},
    {"floor", __vector_floor},
    {"ceil", __vector_ceil},
    {"abs", __vector_abs},
    {"sign", __vector_sign},
    {"clamp", __vector_clamp},
    {"max", __vector_max},
    {"min", __vector_min},
    {"lerp", __vector_lerp},
    {NULL, NULL},
...
static void __createmetatable(lua_State* L)
{
    ...
    lua_pushcfunction(L, __vector_index, NULL);
...
{
    luaL_register(L, LUA_VECLIBNAME, __vectorlib);
