...
const tvalue_t* luaV_tonumber(const tvalue_t* obj, tvalue_t* n)
{
    double num;
    if (ttisnumber(obj))
        return obj;
...
const float* luaV_tovector(const tvalue_t* obj)
{
    if (ttisvector(obj))
        return vvalue(obj);
...
static StkId __callTMres(lua_State* L, StkId res, const tvalue_t* f, const tvalue_t* p1, const tvalue_t* p2)
{
...
static void __callTM(lua_State* L, const tvalue_t* f, const tvalue_t* p1, const tvalue_t* p2, const tvalue_t* p3)
{
    // using stack room beyond top is technically safe here, but for very complicated reasons:
    // * The stack guarantees EXTRA_STACK room beyond stack_last (see luaD_reallocstack) will be allocated
    // * we cannot move luaD_checkstack above because the arguments are *sometimes* pointers to the lua
...
void luaV_gettable(lua_State* L, const tvalue_t* t, tvalue_t* key, StkId val)
{
    int loop;
    for (loop = 0; loop < MAXTAGLOOP; loop++)
    {
        ...
        { // `t' is a table?
            ...
            const tvalue_t* res = luaH_get(h, key); // do a primitive get
        ...
        else if (LUAU_UNLIKELY(FFlag_DebugLuauUserDefinedClassesRuntime && ttisobject(t)))
        {
            luauc_object_t* inst = objectvalue(t);
            const tvalue_t* offsettval = luaH_get(inst->lclass->memberstooffset, key);
        ...
        else if (LUAU_UNLIKELY(FFlag_DebugLuauUserDefinedClassesRuntime && ttisclass(t)))
        {
            luauc_class_t* lco = classvalue(t);
            const tvalue_t* res = luaH_get(lco->memberstooffset, key);
...
void luaV_settable(lua_State* L, const tvalue_t* t, tvalue_t* key, StkId val)
{
    int loop;
    tvalue_t temp;
    for (loop = 0; loop < MAXTAGLOOP; loop++)
    {
        ...
        { // `t' is a table?
            ...
            const tvalue_t* oldval = luaH_get(h, key);
            ...
            if (!ttisnil(oldval) || (tm = fasttm(L, h->metatable, TM_NEWINDEX)) == NULL)
            {
                ...
                tvalue_t* newval = luaH_setslot(L, h, oldval, key);
        ...
        {
            luauc_object_t* inst = objectvalue(t);
            const tvalue_t* offset = luaH_get(inst->lclass->memberstooffset, key);
            if (ttisnil(offset))
                luaG_missingmembererror(L, t, key);
...
static int __call_binTM(lua_State* L, const tvalue_t* p1, const tvalue_t* p2, StkId res, tag_method_t event)
{
    const tvalue_t* tm = luaT_gettmbyobj(L, p1, event); // try first operand
    if (ttisnil(tm))
        tm = luaT_gettmbyobj(L, p2, event); // try second operand
    if (ttisnil(tm))
        return 0;
    __callTMres(L, res, tm, p1, p2);
    return 1;
}

static const tvalue_t* __get_compTM(lua_State* L, lua_table_t* mt1, lua_table_t* mt2, tag_method_t event)
{
    const tvalue_t* tm1 = fasttm(L, mt1, event);
    const tvalue_t* tm2;
    if (tm1 == NULL)
...
static int __call_orderTM(lua_State* L, const tvalue_t* p1, const tvalue_t* p2, tag_method_t event, bool error)
{
    const tvalue_t* tm1 = luaT_gettmbyobj(L, p1, event);
    const tvalue_t* tm2;
    if (ttisnil(tm1))
    {
        if (error)
    ...
    __callTMres(L, L->top, tm1, p1, p2);
    return !l_isfalse(L->top);
}

int luaV_strcmp(const tstring_t* ls, const tstring_t* rs)
{
    if (ls == rs)
        return 0;
...
int luaV_lessthan(lua_State* L, const tvalue_t* l, const tvalue_t* r)
{
    if (LUAU_UNLIKELY(ttype(l) != ttype(r)))
        ...
        return __call_orderTM(L, l, r, TM_LT, /* error= */ true);
}

int luaV_lessequal(lua_State* L, const tvalue_t* l, const tvalue_t* r)
{
    int res;
    if (ttype(l) != ttype(r))
        ...
        return luaV_strcmp(tsvalue(l), tsvalue(r)) <= 0;
    else if ((res = __call_orderTM(L, l, r, TM_LE, false)) != -1) // first try `le'
        return res;
    else if ((res = __call_orderTM(L, r, l, TM_LT, false)) == -1) // error if not `lt'
        luaG_ordererror(L, l, r, TM_LE);
    return !res;
}

int luaV_equalval(lua_State* L, const tvalue_t* t1, const tvalue_t* t2)
{
    const tvalue_t* tm;
    ...
    case LUA_TVECTOR:
        return __luai_veceq(vvalue(t1), vvalue(t2));
    case LUA_TBOOLEAN:
    ...
    case LUA_TUSERDATA:
    {
        tm = __get_compTM(L, uvalue(t1)->metatable, uvalue(t2)->metatable, TM_EQ);
        if (!tm)
    ...
    case LUA_TOBJECT:
    {
        ...
        luauc_object_t* t1inst = objectvalue(t1);
        luauc_object_t* t2inst = objectvalue(t2);
    ...
    case LUA_TTABLE:
    {
        tm = __get_compTM(L, hvalue(t1)->metatable, hvalue(t2)->metatable, TM_EQ);
        if (!tm)
    ...
    __callTMres(L, L->top, tm, t1, t2); // call TM
    return !l_isfalse(L->top);
}
...
void luaV_concat(lua_State* L, int total, int last)
{
    do
    {
        ...
        {
            if (!__call_binTM(L, top - 2, top - 1, top - 2, TM_CONCAT))
...
void luaV_doarithimpl(lua_State* L, StkId ra, const tvalue_t* rb, const tvalue_t* rc, tag_method_t op)
{
    tvalue_t tempb, tempc;
    const tvalue_t *b, *c;
    ...
    const float* vb = ttisvector(rb) ? vvalue(rb) : NULL;
    const float* vc = ttisvector(rc) ? vvalue(rc) : NULL;
    ...
    if (vb && vc)
    {
        ...
        case TM_IDIV:
            setvvalue(
                ra,
                (float)(__luai_numidiv(vb[0], vc[0])),
                (float)(__luai_numidiv(vb[1], vc[1])),
                (float)(__luai_numidiv(vb[2], vc[2])),
    ...
    else if (vb)
    {
        ...
        if (c)
        {
            ...
            case TM_IDIV:
                setvvalue(
                    ra, (float)(__luai_numidiv(vb[0], nc)), (float)(__luai_numidiv(vb[1], nc)), (float)(__luai_numidiv(vb[2], nc)), (float)(luai_numidiv(vb[3], nc))
                );
                return;
            default:
                break;
    ...
    else if (vc)
    {
        ...
        if (b)
        {
            ...
            case TM_IDIV:
                setvvalue(
                    ra, (float)(__luai_numidiv(nb, vc[0])), (float)(__luai_numidiv(nb, vc[1])), (float)(__luai_numidiv(nb, vc[2])), (float)(luai_numidiv(nb, vc[3]))
                );
                return;
            default:
                break;
    ...
    if ((b = luaV_tonumber(rb, &tempb)) != NULL && (c = luaV_tonumber(rc, &tempc)) != NULL)
    {
        ...
        case TM_DIV:
            ...
            break;
        case TM_IDIV:
            setnvalue(ra, __luai_numidiv(nb, nc));
            break;
        case TM_MOD:
...
void luaV_dolen(lua_State* L, StkId ra, const tvalue_t* rb)
{
    const tvalue_t* tm = NULL;
    switch (ttype(rb))
    {
    case LUA_TTABLE:
    {
    ...
    StkId res = __callTMres(L, ra, tm, rb, luaO_nilobject);
    if (!ttisnumber(res))
...
{
    const tvalue_t* tm = luaT_gettmbyobj(L, func, TM_CALL);
    if (!ttisfunction(tm))
