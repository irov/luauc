 ...
 *
 * Current GC stage is stored in global_state_t.gcstate, and has two additional stages for pause and second-phase mark, explained below.
 *
 ...
 * colors are encoded using three bits inside gc_header_t.marked: white0, white1 and black (so technically we use a four-color scheme:
 ...
 * as black (doing so would violate the GC invariant), and they are kept in a special global list (global_state_t.uvhead) which is traversed
...
#define markvalue(g, o) \
    { \
        ...
        if (iscollectable(o) && iswhite(gcvalue(o))) \
            __reallymarkobject(g, gcvalue(o)); \
    }

#define markobject(g, t) \
    { \
        if (iswhite(obj2gco(t))) \
            __reallymarkobject(g, obj2gco(t)); \
    }

#ifdef LUAI_GCMETRICS
static void __recordGcStateStep(global_state_t* g, int startgcstate, double seconds, bool assist, size_t work)
{
    switch (startgcstate)
    {
    case GCSpause:
...
static double __recordGcDeltaTime(double* timer)
{
...
static void __startGcCycleMetrics(global_state_t* g)
{
...
static void __finishGcCycleMetrics(global_state_t* g)
{
    ...
    g->gcmetrics.currcycle = gc_cycle_metrics_t();
...
static void __removeentry(lua_node_t* n)
{
...
static void __reallymarkobject(global_state_t* g, gc_object_t* o)
{
    ...
    case LUA_TUSERDATA:
    {
        lua_table_t* mt = gco2u(o)->metatable;
...
static const char* __gettablemode(global_state_t* g, lua_table_t* h)
{
    const tvalue_t* mode = gfasttm(g, h->metatable, TM_MODE);
...
static int __traversetable(global_state_t* g, lua_table_t* h)
{
    int i;
    int weakkey = 0;
    int weakvalue = 0;
    if (h->metatable)
        markobject(g, cast_to(lua_table_t*, h->metatable));

    // is there a weak mode?
    const char* modev = __gettablemode(g, h);
    if (modev)
    {
    ...
    while (i--)
    {
        lua_node_t* n = gnode(h, i);
        LUAU_ASSERT(ttype(gkey(n)) != LUA_TDEADKEY || ttisnil(gval(n)));
        if (ttisnil(gval(n)))
            __removeentry(n); // remove empty entries
        else
        {
...
*/
static void __traverseproto(global_state_t* g, proto_t* f)
{
    int i;
    if (f->source)
        stringmark(f->source);
...
static void __traverseclosure(global_state_t* g, closure_t* cl)
{
    markobject(g, cl->env);
    if (cl->isC)
    {
    ...
    else
    {
        ...
        markobject(g, cast_to(proto_t*, cl->l.p));
        for (i = 0; i < cl->nupvalues; i++) // mark its upvalues
            markvalue(g, &cl->l.uprefs[i]);
    }
}

static void __traversestack(global_state_t* g, lua_State* l)
{
    markobject(g, l->gt);
    if (l->namecall)
...
static void __traverseclass(global_state_t* g, luauc_class_t* classobject)
{
...
static void __traverseobject(global_state_t* g, luauc_object_t* classinst)
{
...
static void __clearstack(lua_State* l)
{
...
static void __shrinkstack(lua_State* L)
{
...
static void __shrinkstackprotected_run(lua_State* L, void* ud)
{
    (void)ud;
    __shrinkstack(L);
}

static void __shrinkstackprotected(lua_State* L)
{
    // the resize call can fail on exception, in which case we will continue with original size
    int status = luaD_rawrunprotected(L, __shrinkstackprotected_run, NULL);
...
*/
static size_t __propagatemark(global_state_t* g)
{
    gc_object_t* o = g->gray;
    ...
    case LUA_TTABLE:
    {
        lua_table_t* h = gco2h(o);
        g->gray = h->gclist;
        if (__traversetable(g, h)) // table is weak?
            black2gray(o);       // keep it gray

        if (DFFlag_LuauGcTableStepFix)
            return sizeof(lua_table_t) + sizeof(tvalue_t) * h->sizearray + sizeof(lua_node_t) * (h->node == &luaH_dummynode ? 0 : sizenode(h));
        else
            return sizeof(lua_table_t) + sizeof(tvalue_t) * h->sizearray + sizeof(lua_node_t) * sizenode(h);
    }
    case LUA_TFUNCTION:
    {
        closure_t* cl = gco2cl(o);
        g->gray = cl->gclist;
        __traverseclosure(g, cl);
    ...
    case LUA_TTHREAD:
    {
        ...
        if (g->gcstate == GCSpropagate)
            __shrinkstackprotected(th);

        return sizeof(lua_State) + sizeof(tvalue_t) * th->stacksize + sizeof(call_info_t) * th->size_ci;
    }
    case LUA_TPROTO:
    {
        proto_t* p = gco2p(o);
        ...
        return sizeof(proto_t) + sizeof(Instruction) * p->sizecode + sizeof(proto_t*) * p->sizep + sizeof(tvalue_t) * p->sizek + p->sizelineinfo +
               sizeof(local_var_t) * p->sizelocvars + sizeof(tstring_t*) * p->sizeupvalues + p->sizetypeinfo;
    }
    case LUA_TCLASS:
    {
        luauc_class_t* classobject = gco2class(o);
        g->gray = classobject->gclist;
        __traverseclass(g, classobject);
        // We've traversed the "object" itself ...
        return sizeof(luauc_class_t) +
               // ... plus the method closures, each a `tvalue_t` wide ...
               ((classobject->numberofallmembers - classobject->numberofinstancemembers) * sizeof(tvalue_t)) +
               // ... plus a string pointer for each method or property, each a pointer wide.
               (classobject->numberofallmembers * sizeof(tstring_t*));
    }
    case LUA_TOBJECT:
    {
        luauc_object_t* classinst = gco2object(o);
        g->gray = classinst->gclist;
        __traverseobject(g, classinst);
        // We've traversed the instance ...
        return sizeof(luauc_object_t) +
               // ... plus all of the instance fields.
               classinst->numberofmembers * sizeof(tvalue_t);
    }
    default:
        LUAU_ASSERT(0);
        return 0;
...
static size_t __propagateall(global_state_t* g)
{
    size_t work = 0;
    while (g->gray)
    {
        work += __propagatemark(g);
    }
    return work;
}
...
*/
static int __isobjcleared(gc_object_t* o)
{
...
#define iscleared(o) (iscollectable(o) && __isobjcleared(gcvalue(o)))

typedef struct table_resize_context_t
{
    lua_table_t* t;
    int nhsize;
} table_resize_context_t;

static void __tableresizeprotected_run(lua_State* L, void* ud)
{
    table_resize_context_t* ctx = (table_resize_context_t*)ud;
    luaH_resizehash(L, ctx->t, ctx->nhsize);
}

static void __tableresizeprotected(lua_State* L, lua_table_t* t, int nhsize)
{
    table_resize_context_t ctx = {t, nhsize};

    // the resize call can fail on exception, in which case we will continue with original size
    int status = luaD_rawrunprotected(L, __tableresizeprotected_run, &ctx);
    LUAU_ASSERT(status == LUA_OK || status == LUA_ERRMEM);
}
...
*/
static size_t __cleartable(lua_State* L, gc_object_t* l)
{
    size_t work = 0;
    while (l)
    {
        lua_table_t* h = gco2h(l);

        if (DFFlag_LuauGcTableStepFix)
            work += sizeof(lua_table_t) + sizeof(tvalue_t) * h->sizearray + sizeof(lua_node_t) * (h->node == &luaH_dummynode ? 0 : sizenode(h));
        else
            work += sizeof(lua_table_t) + sizeof(tvalue_t) * h->sizearray + sizeof(lua_node_t) * sizenode(h);

        int i = h->sizearray;
        while (i--)
        {
            tvalue_t* o = &h->array[i];
        ...
        while (i--)
        {
            ...
            if (!ttisnil(gval(n)))
            {
                ...
                if (iscleared(gkey(n)) || iscleared(gval(n)))
                {
                    ...
                    __removeentry(n);       // remove entry from table
                }
                else
                {
                    activevalues++;
        ...
        const char* modev = __gettablemode(L->global, h);
        if (modev)
        {
            ...
            if (strchr(modev, 's'))
            {
                ...
                if (activevalues < sizenode(h) * 3 / 8)
                    __tableresizeprotected(L, h, activevalues);
            }
        }
...
static void __freeobj(lua_State* L, gc_object_t* o, lua_page_t* page)
{
    switch (o->gch.tt)
    {
    case LUA_TPROTO:
...
typedef struct string_resize_context_t
{
    int newsize;
} string_resize_context_t;

static void __stringresizeprotected_run(lua_State* L, void* ud)
{
    string_resize_context_t* ctx = (string_resize_context_t*)ud;
    luaS_resize(L, ctx->newsize);
}

static void __stringresizeprotected(lua_State* L, int newsize)
{
    string_resize_context_t ctx = {newsize};

    // the resize call can fail on exception, in which case we will continue with original size
    int status = luaD_rawrunprotected(L, __stringresizeprotected_run, &ctx);
    LUAU_ASSERT(status == LUA_OK || status == LUA_ERRMEM);
}

static void __shrinkbuffers(lua_State* L)
{
    global_state_t* g = L->global;
    // check size of string hash
    if (g->strt.nuse < cast_to(uint32_t, g->strt.size / 4) && g->strt.size > LUA_MINSTRTABSIZE * 2)
        __stringresizeprotected(L, g->strt.size / 2); // table is too big
}

static void __shrinkbuffersfull(lua_State* L)
{
    global_state_t* g = L->global;
    // check size of string hash
    ...
    if (hashsize != g->strt.size)
        __stringresizeprotected(L, hashsize); // table is too big
}

static bool __deletegco(void* context, lua_page_t* page, gc_object_t* gco)
{
    lua_State* L = (lua_State*)context;
    __freeobj(L, gco, page);
    return true;
}
...
static void __markudatadirectfields(global_state_t* g)
{
    LUAU_ASSERT(FFlag_LuauDirectFieldGet);
    for (int i = 0; i < UTAG_INTERNAL_LIMIT; i++)
        if (g->udatadirectfields[i])
            markobject(g, g->udatadirectfields[i]);
}

static void __markmt(global_state_t* g)
{
    int i;
...
static void __marktaggetmt(global_state_t* g)
{
...
// mark root set
static void __markroot(lua_State* L)
{
    global_state_t* g = L->global;
    g->gray = NULL;
    g->grayagain = NULL;
    ...
    if (FFlag_LuauDirectFieldGet)
        __markudatadirectfields(g);

    __markmt(g);

    if (FFlag_LuauUdataMetatablePinned)
        __marktaggetmt(g);

    g->gcstate = GCSpropagate;
}

static size_t __remarkupvals(global_state_t* g)
{
    size_t work = 0;

    for (upvalue_t* uv = g->uvhead.u.open.next; uv != &g->uvhead; uv = uv->u.open.next)
    {
        work += sizeof(upvalue_t);
...
static size_t __clearupvals(lua_State* L)
{
    global_state_t* g = L->global;

    size_t work = 0;

    for (upvalue_t* uv = g->uvhead.u.open.next; uv != &g->uvhead;)
    {
        work += sizeof(upvalue_t);
...
static size_t __atomic(lua_State* L)
{
    global_state_t* g = L->global;
    ...
    work += __remarkupvals(g);
    // traverse objects caught by write barrier and by 'remarkupvals'
    work += __propagateall(g);

#ifdef LUAI_GCMETRICS
    g->gcmetrics.currcycle.atomictimeupval += __recordGcDeltaTime(&currts);
#endif
    ...
    __markmt(g);        // mark basic metatables (again)

    if (FFlag_LuauUdataMetatablePinned)
        __marktaggetmt(g); // mark tagged userdata metatables (again)

    if (FFlag_LuauDirectFieldGet)
        __markudatadirectfields(g); // mark direct field dispatch tables (again)

    work += __propagateall(g);

#ifdef LUAI_GCMETRICS
    g->gcmetrics.currcycle.atomictimeweak += __recordGcDeltaTime(&currts);
#endif
...
#ifdef LUAI_GCMETRICS
    g->gcmetrics.currcycle.atomictimegray += __recordGcDeltaTime(&currts);
#endif

    // remove collected objects from weak tables
    work += __cleartable(L, g->weak);
    g->weak = NULL;

#ifdef LUAI_GCMETRICS
    g->gcmetrics.currcycle.atomictimeclear += __recordGcDeltaTime(&currts);
#endif

    // close orphaned live upvalues of dead threads and clear dead upvalues
    work += __clearupvals(L);
...
static int __sweepgcopage(lua_State* L, lua_page_t* page)
{
    char* start;
    char* end;
    int busyBlocks;
...
static size_t __gcstep(lua_State* L, size_t limit)
{
    size_t cost = 0;
    global_state_t* g = L->global;
    switch (g->gcstate)
    ...
    case GCSpause:
    {
        __markroot(L); // start a new collection
    ...
    case GCSatomic:
    {
        ...
        cost = __atomic(L); // finish mark phase
    ...
    case GCSsweep:
    {
        ...
        {
            lua_page_t* next = luaM_getnextpage(g->sweepgcopage); // page sweep might destroy the page

            int steps = __sweepgcopage(L, g->sweepgcopage);
...
static int64_t __getheaptriggererroroffset(global_state_t* g)
{
...
static size_t __getheaptrigger(global_state_t* g, size_t heapgoal)
{
    ...
    int64_t offset = __getheaptriggererroroffset(g);
...
size_t luaC_step(lua_State* L, bool assist)
{
    ...
    size_t work = __gcstep(L, lim);

#ifdef LUAI_GCMETRICS
    __recordGcStateStep(g, lastgcstate, lua_clock() - lasttimestamp, assist, work);
#endif
    ...
    if (g->gcstate == GCSpause)
    {
        ...
        size_t heaptrigger = __getheaptrigger(g, heapgoal);
...
void luaC_barrierf(lua_State* L, gc_object_t* o, gc_object_t* v)
{
    global_state_t* g = L->global;
    ...
    // must keep invariant?
    if (keepinvariant(g))
        __reallymarkobject(g, v); // restore invariant
    else                        // don't mind
        makewhite(g, o);        // mark as white just to avoid other barriers
}

void luaC_barriertable(lua_State* L, lua_table_t* t, gc_object_t* v)
{
    global_state_t* g = L->global;
    gc_object_t* o = obj2gco(t);

    // in the second propagation stage, table assignment barrier works as a forward barrier
    if (g->gcstate == GCSpropagateagain)
    {
        LUAU_ASSERT(isblack(o) && iswhite(v) && !isdead(g, v) && !isdead(g, o));
        __reallymarkobject(g, v);
        return;
    }
...
void luaC_barrierback(lua_State* L, gc_object_t* o, gc_object_t** gclist)
{
    global_state_t* g = L->global;
    LUAU_ASSERT(isblack(o) && !isdead(g, o));
    LUAU_ASSERT(g->gcstate != GCSpause);
...
void luaC_upvalclosed(lua_State* L, upvalue_t* uv)
{
    global_state_t* g = L->global;
