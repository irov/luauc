...
static int __foreachi(lua_State* L)
{
...
static int __foreach (lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checktype(L, 2, LUA_TFUNCTION);
...
static int __maxn(lua_State* L)
{
    double max = 0;
    luaL_checktype(L, 1, LUA_TTABLE);

    lua_table_t* t = hvalue(L->base);
...
static void __moveelements(lua_State* L, int srct, int dstt, int f, int e, int t)
{
    lua_table_t* src = hvalue(L->base + (srct - 1));
    lua_table_t* dst = hvalue(L->base + (dstt - 1));
    ...
    {
        tvalue_t* srcarray = src->array;
        tvalue_t* dstarray = dst->array;

        if (t > e || t <= f || (dstt != srct && dst != src))
        {
            for (int i = 0; i < n; ++i)
            {
                tvalue_t* s = &srcarray[f + i - 1];
                tvalue_t* d = &dstarray[t + i - 1];
                setobj2t(L, d, s);
            }
        }
        else
...
static int __tinsert(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    int n = lua_objlen(L, 1);
    ...
    case 3:
    {
        ...
        if (1 <= pos && pos <= n)
            __moveelements(L, 1, 1, pos, n, pos + 1);
        break;
    }
    default:
    {
...
static int __tremove(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    int n = lua_objlen(L, 1);
    ...
    __moveelements(L, 1, 1, pos + 1, n, pos);
...
*/
static int __tmove(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    ...
    { // otherwise, nothing to move
        ...
        lua_table_t* dst = hvalue(L->base + (tt - 1));
        ...
        __moveelements(L, 1, tt, f, e, t);
    }
    lua_pushvalue(L, tt); // return destination table
    return 1;
}

static void __addfield(lua_State* L, luaL_Strbuf* b, int i, lua_table_t* t)
{
    if (t && ((unsigned)(i - 1)) < ((unsigned)(t->sizearray)) && ttisstring(&t->array[i - 1]))
    {
        tstring_t* ts = tsvalue(&t->array[i - 1]);
...
static int __tconcat(lua_State* L)
{
    size_t lsep;
...
static int __tpack(lua_State* L)
{
    int n = lua_gettop(L);    // number of elements to pack
    lua_createtable(L, n, 1); // create result table

    lua_table_t* t = hvalue(L->top - 1);

    for (int i = 0; i < n; ++i)
    {
        tvalue_t* e = &t->array[i];
        setobj2t(L, e, L->base + i);
    }

    // t.n = number of elements
    tvalue_t* nv = luaH_setstr(L, t, luaS_newliteral(L, "n"));
    setnvalue(nv, n);

    return 1; // return table
}

static int __tunpack(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    lua_table_t* t = hvalue(L->base);
...
typedef int (*SortPredicate)(lua_State* L, const tvalue_t* l, const tvalue_t* r);

static int __sort_func(lua_State* L, const tvalue_t* l, const tvalue_t* r)
{
...
static inline void __sort_swap(lua_State* L, lua_table_t* t, int i, int j)
{
    tvalue_t* arr = t->array;
    int n = t->sizearray;
...
static inline int __sort_less(lua_State* L, lua_table_t* t, int i, int j, SortPredicate pred)
{
    tvalue_t* arr = t->array;
    int n = t->sizearray;
    LUAU_ASSERT(((unsigned)(i)) < ((unsigned)(n)) && ((unsigned)(j)) < ((unsigned)(n))); // contract maintained in sort_less after predicate call
...
static void __sort_siftheap(lua_State* L, lua_table_t* t, int l, int u, SortPredicate pred, int root)
{
    LUAU_ASSERT(l <= u);
    int count = u - l + 1;
    ...
    while (root * 2 + 2 < count)
    {
        ...
        int next = root;
        next = __sort_less(L, t, l + next, l + left, pred) ? left : next;
        next = __sort_less(L, t, l + next, l + right, pred) ? right : next;

        if (next == root)
            break;

        __sort_swap(L, t, l + root, l + next);
        root = next;
    }

    // process last element if it has just one child
    int lastleft = root * 2 + 1;
    if (lastleft == count - 1 && __sort_less(L, t, l + root, l + lastleft, pred))
        __sort_swap(L, t, l + root, l + lastleft);
}

static void __sort_heap(lua_State* L, lua_table_t* t, int l, int u, SortPredicate pred)
{
    LUAU_ASSERT(l <= u);
    int count = u - l + 1;

    for (int i = count / 2 - 1; i >= 0; --i)
        __sort_siftheap(L, t, l, u, pred, i);

    for (int i = count - 1; i > 0; --i)
    {
        __sort_swap(L, t, l, l + i);
        __sort_siftheap(L, t, l, l + i - 1, pred, 0);
    }
}

static void __sort_rec(lua_State* L, lua_table_t* t, int l, int u, int limit, SortPredicate pred)
{
    ...
    while (l < u)
    {
        ...
        if (limit == 0)
        {
            __sort_heap(L, t, l, u, pred);
            return;
        }

        // sort elements a[l], a[(l+u)/2] and a[u]
        // note: this simultaneously acts as a small sort and a median selector
        if (__sort_less(L, t, u, l, pred)) // a[u] < a[l]?
            __sort_swap(L, t, u, l);       // swap a[l] - a[u]
        if (u - l == 1)
            break;                       // only 2 elements
        int m = l + ((u - l) >> 1);      // midpoint
        if (__sort_less(L, t, m, l, pred)) // a[m]<a[l]?
            __sort_swap(L, t, m, l);
        else if (__sort_less(L, t, u, m, pred)) // a[u]<a[m]?
            __sort_swap(L, t, m, u);
        if (u - l == 2)
        ...
        int p = u - 1;
        __sort_swap(L, t, m, u - 1); // pivot is now (and always) at u-1
        ...
        { // invariant: a[l..i] <= P <= a[j..u]
            ...
            while (__sort_less(L, t, ++i, p, pred))
            {
                if (i >= u)
                    luaL_error(L, "invalid order function for sorting");
            }
            // repeat --j until a[j] <= P
            while (__sort_less(L, t, p, --j, pred))
            {
                if (j <= l)
                    luaL_error(L, "invalid order function for sorting");
            }
        ...
        if (i - l < u - i)
        {
            __sort_rec(L, t, l, i - 1, limit, pred);
            l = i + 1;
        }
        else
        {
            __sort_rec(L, t, i + 1, u, limit, pred);
            u = i - 1;
        }
    }
}

static int __tsort(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    lua_table_t* t = hvalue(L->base);
    int n = luaH_getn(t);
    ...
    if (n > 0)
        __sort_rec(L, t, 0, n - 1, n, pred);
    return 0;
}

static int __tcreate(lua_State* L)
{
...
static int __tfind(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checkany(L, 2);
    ...
    for (int i = init;; ++i)
    {
        const tvalue_t* e = luaH_getnum(t, i);
        if (ttisnil(e))
            break;
...
static int __tclear(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);

    lua_table_t* tt = hvalue(L->base);
    if (tt->readonly)
        luaG_readonlyerror(L);
...
static int __tfreeze(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
...
static int __tisfrozen(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
...
static int __tclone(lua_State* L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_argcheck(L, !luaL_getmetafield(L, 1, "__metatable"), 1, "table has a protected metatable");

    lua_table_t* tt = luaH_clone(L, hvalue(L->base));
...
static const luaL_Reg __tab_funcs[] = {
    {"concat", __tconcat},
    {"foreach", __foreach},
    {"foreachi", __foreachi},
    {"getn", __getn},
...
{
    luaL_register(L, LUA_TABLIBNAME, __tab_funcs);

    // Lua 5.1 compat
    lua_pushcfunction(L, __tunpack, "unpack");
