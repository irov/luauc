...
udata_t* luaU_newudata(lua_State* L, size_t s, int tag)
{
    if (s > INT_MAX - sizeof(udata_t))
        luaM_toobig(L);
    udata_t* u = luaM_newgco(L, udata_t, sizeudata(s), L->activememcat);
...
void luaU_freeudata(lua_State* L, udata_t* u, lua_page_t* page)
{
