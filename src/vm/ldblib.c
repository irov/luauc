...
static lua_State* __getthread(lua_State* L, int* arg)
{
    if (lua_isthread(L, 1))
    {
        *arg = 1;
...
static int __db_info(lua_State* L)
{
    int arg;
    lua_State* L1 = __getthread(L, &arg);
    int l1top = 0;
...
static int __db_traceback(lua_State* L)
{
    int arg;
    lua_State* L1 = __getthread(L, &arg);
...
static const luaL_Reg __dblib[] = {
    {"info", __db_info},
    {"traceback", __db_traceback},
    {NULL, NULL},
};

int luaopen_debug(lua_State* L)
{
    luaL_register(L, LUA_DBLIBNAME, __dblib);
    return 1;
}
