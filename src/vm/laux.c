...
typedef struct lua_pcall_yieldable_context_t
{
    StkId function;
    int result_count;
} lua_pcall_yieldable_context_t;

static void __lua_pcall_yieldable_run(lua_State* L, void* userdata)
{
    lua_pcall_yieldable_context_t* context = (lua_pcall_yieldable_context_t*)userdata;
    luaD_callint(L, context->function, context->result_count, lua_isyieldable(L) != 0);
}
...
void* luaL_checkbuffer(lua_State* L, int narg, size_t* len)
{
    ...
    if (!b)
        __tag_error(L, narg, LUA_TBUFFER);
    return b;
}
...
const char* luaL_checklstring(lua_State* L, int narg, size_t* len)
{
    ...
    if (!s)
        __tag_error(L, narg, LUA_TSTRING);
    return s;
}
...
double luaL_checknumber(lua_State* L, int narg)
{
    ...
    if (!isnum)
        __tag_error(L, narg, LUA_TNUMBER);
    return d;
}
...
int luaL_checkboolean(lua_State* L, int narg)
{
    ...
    if (!lua_isboolean(L, narg))
        __tag_error(L, narg, LUA_TBOOLEAN);
...
int64_t luaL_checkinteger64(lua_State* L, int narg)
{
    if (!lua_isinteger64(L, narg))
        __tag_error(L, narg, LUA_TINTEGER);
    return lua_tointeger64(L, narg, NULL);
}
...
const float* luaL_checkvector(lua_State* L, int narg)
{
    ...
    if (!v)
        __tag_error(L, narg, LUA_TVECTOR);
    return v;
}
...
static int __libsize(const luaL_Reg* l)
{
    int size = 0;
    for (; l->name; l++)
        size++;
...
{
    const tvalue_t* obj = luaA_toobject(L, idx);
...
int luaL_pcallyieldable(lua_State* L, int nargs, int nresults, int errfunc)
{
    ...
    lua_pcall_yieldable_context_t context = {L->top - (nargs + 1), nresults};

    ptrdiff_t savedfunc = savestack(L, context.function);
    ptrdiff_t savederrfunc = errfunc != 0 ? savestack(L, L->base + (errfunc - 1)) : 0;

    int status = luaD_pcall(L, __lua_pcall_yieldable_run, &context, savedfunc, savederrfunc);
...
/*
** {======================================================
** Generic luauc_vm_buffer_t manipulation
** =======================================================
*/

static size_t __getnextbuffersize(lua_State* L, size_t currentsize, size_t desiredsize)
{
...
static char* __extendstrbuf(luaL_Strbuf* B, size_t additionalsize, int boxloc)
{
    lua_State* L = B->L;
    ...
    size_t nextsize = __getnextbuffersize(B->L, capacity, capacity + additionalsize);

    tstring_t* newStorage = luaS_bufstart(L, nextsize);
...
char* luaL_prepbuffsize(luaL_Strbuf* B, size_t size)
{
    if (((size_t)(B->end - B->p)) < size)
        return __extendstrbuf(B, size - (B->end - B->p), -1);
    return B->p;
}
...
void luaL_addlstring(luaL_Strbuf* B, const char* s, size_t len)
{
    if (((size_t)(B->end - B->p)) < len)
        __extendstrbuf(B, len - (B->end - B->p), -1);
...
void luaL_addvalue(luaL_Strbuf* B)
{
    ...
    size_t vl;
    const char* string = lua_tolstring(L, -1, &vl);
    if (string != NULL)
    {
        if (((size_t)(B->end - B->p)) < vl)
            __extendstrbuf(B, vl - (B->end - B->p), -2);

        memcpy(B->p, string, vl);
        B->p += vl;
...
const char* luaL_tolstring(lua_State* L, int idx, size_t* len)
{
    ...
    case LUA_TINTEGER:
    {
        int64_t l = lua_tointeger64(L, idx, NULL);
        char s[LUAI_MAXINT2STR];
